...
public class NetworkManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!serverFound)
        {
            ...
            if (!udpClient.noNetwork)
            {
                ...
                CanvasManager.instance.txtSearchServerStatus.text = "Please start the server ";
    ...
    public void OnPrintPongMsg(UDPEvent data)
    {
		...
		*/
        ...
        CanvasManager.instance.txtSearchServerStatus.text = "Connected to server!";
    ...
    void OnReceiveString(UDPEvent data)
    {
		 ...
		 * data.pack[1] = the message
		*/

        CanvasManager.instance.ShowAlertDialog("string received!");

        CanvasManager.instance.ShowString(data.pack[1]);


    }


    //Envia para o UDPServer o comando para abrir um aplicativo nos dispositivos clientes
    public void SendString()
    {
        // make sure we have both modules ready to send GPS data
        if (UDPServerModule.UDPServer.instance.readyForGPS)
        {
            if (GPS_Manager.instance.GPS_ready)
            {
                ...
                //JSON package
                data["heading"] = GPS_Manager.instance.deviceHEADING.ToString();
                data["lat"] = GPS_Manager.instance.deviceLAT.ToString();
                data["lng"] = GPS_Manager.instance.deviceLNG.ToString();

                string dataMessage = data["heading"] + ":" + data["lat"] + ":" + data["lng"];
                data["msg"] = dataMessage;

                //The Emit method sends the mapped callback name to  the server
                udpClient.EmitToServer("SEND_STRING", data["msg"]);
            }
        }
    }
