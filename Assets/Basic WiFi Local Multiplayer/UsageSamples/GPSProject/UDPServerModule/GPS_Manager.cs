    ...
    void Update()
    {
        ...
        else
        {
            if (Input.location.status != LocationServiceStatus.Failed)
            {
                ...
                deviceHEADING = (int)Input.compass.trueHeading;

                // if compass isnt yet enabled, enable it
                if (!Input.compass.enabled)
                    Input.compass.enabled = true;

                HeadingText.text = deviceHEADING.ToString();
                LatText.text = deviceLAT.ToString();
                LngText.text = deviceLNG.ToString();

                NetworkManager.instance.SendString();
